...
//		int[][][] lu.zhang.dp = new int[N + 1][M + 1][sum + 1];
//		for (int i = 0; i <= N; i++) {
//			for (int j = 0; j <= M; j++) {
//				for (int k = 0; k <= sum; k++) {
//					lu.zhang.dp[i][j][k] = -1;
...
//		if ((arr.length & 1) != 0) {
//			for (int k = 0; k <= sum; k++) {
//				lu.zhang.dp[N][M - 1][k] = 0;
//			}
//		}
//		for (int i = N - 1; i >= 0; i--) {
//			for (int picks = 0; picks <= M; picks++) {
//				for (int rest = 0; rest <= sum; rest++) {
//					int p1 = lu.zhang.dp[i + 1][picks][rest];
//					int p2 = -1;
//					int next2 = -1;
//					if (picks + 1 <= M && arr[i] <= rest) {
//						next2 = lu.zhang.dp[i + 1][picks + 1][rest - arr[i]];
//					}
//					if (next2 != -1) {
//						p2 = arr[i] + next2;
//					}
//					lu.zhang.dp[i][picks][rest] = Math.max(p1, p2);
//				}
//			}
//		}
//		return lu.zhang.dp[0][0][sum];
//	}
