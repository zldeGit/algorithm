...
public class Code07_TargetSum {
  ...
  public static int process1(int[] arr, int index, int rest) {
    ...
    return process1(arr, index + 1, rest - arr[index]) + process1(arr, index + 1,
        rest + arr[index]);
  }
  ...
  public static int process2(int[] arr, int index, int rest,
    ...
    if (index == arr.length) {
      ...
      ans = process2(arr, index + 1, rest - arr[index], dp) + process2(arr, index + 1,
          rest + arr[index], dp);
    }
  ...
  public static int subset1(int[] nums, int s) {
    ...
    int n = nums.length;
    // lu.zhang.dp[i][j] : nums前缀长度为i的所有子集，有多少累加和是j？
  ...
  // 因为如果 i - n < 0，lu.zhang.dp[i]怎么更新？和上一步的dp[i]一样！所以不用更新
  // 如果 i - n >= 0，lu.zhang.dp[i]怎么更新？上一步的dp[i] + 上一步dp[i - n]的值，这才需要更新
