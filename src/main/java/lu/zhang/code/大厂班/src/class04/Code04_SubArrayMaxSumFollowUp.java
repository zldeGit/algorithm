package lu.zhang.code.大厂班.src.class04;
...
public class Code04_SubArrayMaxSumFollowUp {
  ...
  public static int subSqeMaxSumNoNext(int[] arr) {
    ...
    // lu.zhang.dp[i] : arr[0..i]挑选，满足不相邻设定的情况下，随意挑选，最大的累加和
    dp[0] = arr[0];
  ...
  // 可能性 1) 选出的组合，不包含arr[i]。那么dp[i] = lu.zhang.dp[i-1]
  // 比如，arr[0...i] = {3,4,-4}，最大累加和是不包含i位置数的时候
  //
  // 可能性 2) 选出的组合，只包含arr[i]。那么dp[i] = arr[i]
  // 比如，arr[0...i] = {-3,-4,4}，最大累加和是只包含i位置数的时候
  //
  // 可能性 3) 选出的组合，包含arr[i], 且包含arr[0...i-2]范围上的累加和。那么dp[i] = arr[i] + lu.zhang.dp[i-2]
  // 比如，arr[0...i] = {3,1,4}，最大累加和是3和4组成的7，因为相邻不能选，所以i-1位置的数要跳过
  //
  // 综上所述：lu.zhang.dp[i] = Max { lu.zhang.dp[i-1], arr[i] , arr[i] + lu.zhang.dp[i-2] }
