package lu.zhang.code.大厂班.src.class05;
...
public class Code04_DeleteMinCost {
  ...
  public static int distance(char[] str2, char[] s1sub) {
    ...
    // 那么就是通过str2[0..i-1]变成s1sub[0..j]之后，再最后删掉str2[i]即可 -> lu.zhang.dp[i][j] = lu.zhang.dp[i-1][j] + 1
    // 可能性二：
    // str2[0..i]变的过程中，想保留最后一个字符(str2[i])，然后变成s1sub[0..j]，
    // 这要求str2[i] == s1sub[j]才有这种可能, 然后str2[0..i-1]变成s1sub[0..j-1]即可
    // 也就是str2[i] == s1sub[j] 的条件下，lu.zhang.dp[i][j] = lu.zhang.dp[i-1][j-1]
