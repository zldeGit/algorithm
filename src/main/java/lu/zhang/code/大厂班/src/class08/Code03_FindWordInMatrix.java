...
public class Code03_FindWordInMatrix {
  ...
  public static boolean findWord1(char[][] m, String word) {
    ...
    int M = m[0].length;
    int len = w.length;
    // lu.zhang.dp[i][j][k]表示：必须以m[i][j]这个字符结尾的情况下，能不能找到w[0...k]这个前缀串
  ...
  public static boolean canLoop(char[][] m, int i, int j, char[] str, int k) {
    ...
    // str[k+1....]
    boolean ans = false;
    if (canLoop(m, i + 1, j, str, k + 1) || canLoop(m, i - 1, j, str, k + 1) || canLoop(m, i, j + 1,
        str, k + 1)
  ...
  public static boolean noLoop(char[][] m, int i, int j, char[] str, int k) {
    ...
    m[i][j] = 0;
    boolean ans = false;
    if (noLoop(m, i + 1, j, str, k + 1) || noLoop(m, i - 1, j, str, k + 1) || noLoop(m, i, j + 1,
        str, k + 1)
  ...
  public static boolean process(char[][] m, int i, int j, char[] str, int k) {
    ...
    if (i == -1 || i == m.length || j == -1 || j == m[0].length || m[i][j] == 0
        || m[i][j] != str[k]) {
      return false;
    }
    m[i][j] = 0;
    boolean ans = false;
    if (process(m, i + 1, j, str, k + 1) || process(m, i - 1, j, str, k + 1) || process(m, i, j + 1,
        str, k + 1)
