package lu.zhang.code.大厂班.src.class13;
...
public class Code03_ScrambleString {
  ...
  public static boolean isScramble2(String s1, String s2) {
    ...
    // lu.zhang.dp[i][j][k] = 0 processDP(i,j,k)状态之前没有算过的
    // lu.zhang.dp[i][j][k] = -1 processDP(i,j,k)状态之前算过的,返回值是false
    // lu.zhang.dp[i][j][k] = 1 processDP(i,j,k)状态之前算过的,返回值是true
        ...
        for (int L2 = 0; L2 <= N - size; L2++) {
          for (int leftPart = 1; leftPart < size; leftPart++) {
            if ((dp[L1][L2][leftPart] && dp[L1 + leftPart][L2 + leftPart][size - leftPart])
                || (dp[L1][L2 + size - leftPart][leftPart] && dp[L1 + leftPart][L2][size
                - leftPart])) {
              dp[L1][L2][size] = true;
              break;
            }
