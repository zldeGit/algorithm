package lu.zhang.code.大厂班.src.class16;
...
public class Code01_IsSum {
  ...
  public static boolean process2(int[] arr, int i, int sum,
  ...
  public static boolean isSum3(int[] arr, int sum) {
    ...
    // -7  -6  -5  -4  -3   -2  -1   0（想象中）
    //
    // lu.zhang.dp[0][-min] -> lu.zhang.dp[0][7] -> lu.zhang.dp[0][0]
    boolean[][] dp = new boolean[N][max - min + 1];
    // lu.zhang.dp[0][0] = true
    dp[0][-min] = true;
    // lu.zhang.dp[0][arr[0]] = true
    dp[0][arr[0] - min] = true;
    for (int i = 1; i < N; i++) {
      for (int j = min; j <= max; j++) {
        // lu.zhang.dp[i][j] = lu.zhang.dp[i-1][j]
        dp[i][j - min] = dp[i - 1][j - min];
        // lu.zhang.dp[i][j] |= lu.zhang.dp[i-1][j - arr[i]]
