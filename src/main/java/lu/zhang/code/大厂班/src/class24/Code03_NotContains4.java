package lu.zhang.code.大厂班.src.class24;
...
public class Code03_NotContains4 {
  ...
  public static long[] arr = {0L, 1L, 9L, 81L, 729L, 6561L, 59049L, 531441L, 4782969L, 43046721L,
      387420489L,
  ...
  public static long notContains4Nums2(long num) {
    ...
    return arr[len] - 1 + (first - (first < 4 ? 1 : 2)) * arr[len] + process(num % offset,
        offset / 10, len - 1);
  }
  ...
  public static long process(long num, long offset, int len) {
    ...
    long first = num / offset;
    return (first < 4 ? first : (first - 1)) * arr[len] + process(num % offset, offset / 10,
        len - 1);
  }
