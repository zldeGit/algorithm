package lu.zhang.code.大厂班.src.class44;
...
public class Problem_0317_ShortestDistanceFromAllBuildings {
  ...
  public static void add(Queue<Position> q, HashMap<Position, Integer> l, Position[][] p, int i,
      int j, int level) {
    if (i >= 0 && i < p.length && j >= 0 && j < p[0].length && p[i][j].v != 2 && !l.containsKey(
        p[i][j])) {
      l.put(p[i][j], level);
      q.add(p[i][j]);
    }
  ...
  public static void add(Queue<Info> q, HashMap<Info, Integer> l, Info[][] infos, int i, int j,
      int level) {
